...
exports.cancelAppointment = async (req, res) => {
  try {
    ...
    // Free up slot in doctor's calendar
    const doctor = await Doctor.findById(appointment.doctorId);
    if (doctor) {
      const calendarEntry = doctor.calendar.find(entry =>
        entry.date.toISOString().split('T')[0] === appointment.date.toISOString().split('T')[0]
      );
      if (calendarEntry) {
        const slot = calendarEntry.availableSlots.find(slot =>
...
exports.updateTimeSlot = async (req, res) => {
  try {
    ...
    // Free old slot
    const oldEntry = doctor.calendar.find(entry =>
      entry.date.toISOString().split('T')[0] === appointment.date.toISOString().split('T')[0]
    );
    if (oldEntry) {
      const oldSlot = oldEntry.availableSlots.find(slot =>
        slot.startTime.getTime() === appointment.startTime.getTime() &&
        slot.endTime.getTime() === appointment.endTime.getTime()
      );
    ...
    // Find new slot
    const newEntry = doctor.calendar.find(entry =>
    ...
    const newSlot = newEntry.availableSlots.find(slot =>
